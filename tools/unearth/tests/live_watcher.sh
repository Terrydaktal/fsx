...
CLIENT_BIN="${UNEARTH_BIN:-${ROOT_DIR}/target/debug/unearth}"
DAEMON_BIN="${UNEARTHD_BIN:-${ROOT_DIR}/target/debug/unearthd}"
...
if [[ ! -x "$CLIENT_BIN" || ! -x "$DAEMON_BIN" ]]; then
  cargo build --quiet --manifest-path "${ROOT_DIR}/Cargo.toml" --features watcher --bin unearth --bin unearthd
fi
